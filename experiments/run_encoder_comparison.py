#!/usr/bin/env python3
"""
Experiment 7: Encoder Type Comparison
=======================================
Tests different encoder architectures for GAHIB with the same loss config
(IB + Lorentz). Isolates the effect of the encoder backbone.

Variants (3):
  1. MLP          — Standard MLP encoder (mini-batch)
  2. GAT          — Graph Attention Network encoder [GAHIB default]
  3. Transformer  — Self-attention Transformer encoder

All share: recon=1.0, irecon=0.5, lorentz=5.0, beta=0.1, loss_type=nb.
...
# Shared loss config — same for all encoders
_BASE_LOSS = dict(
    recon=1.0, irecon=0.5, lorentz=5.0, beta=0.1,
)

VARIANTS = {
    'MLP': dict(**_BASE_LOSS, encoder_type='mlp'),
    'GAT': dict(**_BASE_LOSS, encoder_type='graph', graph_type='GAT'),
    'Transformer': dict(**_BASE_LOSS, encoder_type='transformer',
                        attn_embed_dim=64, attn_num_heads=4,
                        attn_num_layers=2, attn_seq_len=8),
}
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"GAHIB ENCODER TYPE COMPARISON")
    ...
    print(f"\n{'='*70}")
    print(f"ENCODER COMPARISON COMPLETE — Results in: {TABLES_DIR}")
    print(f"{'='*70}")
