% ============================================================================
% GAHIB: Graph Attention VAE with a Hyperbolic Information Bottleneck
...
% Tighten the gap between a figure/table and its caption, and after the caption
...
\title{GAHIB: Graph Attention VAE with a Hyperbolic\\
Information Bottleneck for Biologically Structured\\
Single-Cell Representations}

\author[amu]{Zeyu Fu\textsuperscript{$\ast$,$\dagger$}}
\ead{fuzeyu99@126.com}

\author[xinqiao]{Jiawei Fu\textsuperscript{$\dagger$}}
\ead{fjw813130855@163.com}

\author[cqmu]{Xiaoxia Wang\textsuperscript{$\dagger$}}
\ead{wxxxxxx96@163.com}

\author[amutox]{Yiyao Liu\textsuperscript{$\ast$}}
\ead{2415084056@qq.com}

\author[xinqiao]{Tianfei Ran\textsuperscript{$\ast$}}
\ead{rantianfei@tmmu.edu.cn}

\address[amu]{State Key Laboratory of Trauma and Chemical Poisoning,
Institute of Combined Injury, Chongqing Engineering Research Center for
Nanomedicine, College of Preventive Medicine, Army Medical University,
Chongqing, 400038, China}
\address[xinqiao]{Department of Orthopedics, Xinqiao Hospital,
Army Medical University, Chongqing, 400037, China}
\address[cqmu]{Department of Hepatobiliary Surgery, The Second Affiliated
Hospital of Chongqing Medical University, Chongqing, 400037, China}
\address[amutox]{State Key Laboratory of Trauma and Chemical Poisoning,
Key Laboratory of Medical Protection for Electromagnetic Radiation,
Ministry of Education of China, Institute of Toxicology, College of
Preventive Medicine, Army Medical University, Chongqing, 400038, China}

\address[note]{%
  \textsuperscript{$\ast$}Corresponding authors: Zeyu Fu
  (\href{mailto:fuzeyu99@126.com}{fuzeyu99@126.com};
  ORCID \href{https://orcid.org/0009-0001-8329-0108}{0009-0001-8329-0108}),
  Yiyao Liu
  (\href{mailto:2415084056@qq.com}{2415084056@qq.com}),
  Tianfei Ran
  (\href{mailto:rantianfei@tmmu.edu.cn}{rantianfei@tmmu.edu.cn}).
  \textsuperscript{$\dagger$}Zeyu Fu, Jiawei Fu and Xiaoxia Wang
  contributed equally to this work.}

% ── Abstract ──────────────────────────────────────────────────────────────
\begin{abstract}
Current single-cell RNA-sequencing (scRNA-seq) variational
autoencoders typically model either local cell-graph structure,
hyperbolic latent geometry, or bottleneck compression, but not all
three within one variational objective. We present \textbf{GAHIB}
(\emph{Graph Attention VAE with a Hyperbolic Information
Bottleneck}), which combines a graph-attention encoder, a 2D
information bottleneck, and a Lorentz-hyperbolic geometry loss.
The bottleneck provides the low-dimensional target used by the
hyperbolic term, while the graph encoder incorporates neighbourhood
structure into the learned representation. Across 53 scRNA-seq
datasets, 11 studies, and 20 metrics, GAHIB matches or improves on
deep-learning, classical, geometric-VAE, and disentanglement
baselines for clustering, embedding quality, and latent-structure
metrics, with paired Wilcoxon signed-rank significance in most
pairwise comparisons after Benjamini--Hochberg correction. Ablation
analyses indicate that each component contributes measurable signal,
and on lineage-annotated datasets the latent space shows radial
ordering consistent with developmental progression. Performance
remains stable across random seeds and hyperparameter sweeps, and
runtime is practical under the tested settings.
\end{abstract}
...
% Highlights and key research questions have been moved to a dedicated
% standalone document: ../highlights/highlights.tex
...
Single-cell RNA-sequencing (scRNA-seq) now routinely resolves
transcriptional state at the resolution of individual cells, yielding
high-dimensional count matrices with thousands of genes per
cell~\cite{zheng2017massively}. Almost every downstream question asked
of these data---clustering into cell types, inferring trajectories,
detecting differential expression, annotating states---reduces to a
question about the \emph{latent representation} used to summarise
each cell. Widely adopted pipelines such as
Scanpy~\cite{wolf2018scanpy} and Leiden
clustering~\cite{traag2019leiden} inherit the strengths and weaknesses
of whichever embedding is fed into them, so the choice of latent
space is consequential far beyond the embedding step
itself~\cite{luecken2019bestpractices,lahnemann2020grandchallenges}.

Unlike bulk expression measurements, scRNA-seq matrices are sparse,
overdispersed, and well approximated by negative-binomial count
models~\cite{hafemeister2019sctransform,svensson2020droplet}.
They are also structurally heterogeneous: discrete cell identities
coexist with continuous transitional states and branching developmental
programmes that standard clustering
alone cannot resolve~\cite{trapnell2014monocle,haghverdi2016dpt}.
An embedding useful in practice must therefore satisfy several
requirements simultaneously.
It must denoise counts, preserve local neighbour relations for
clustering and graph construction, and retain enough global
organisation for reliable trajectory
inference~\cite{saelens2019trajectory} and biological interpretation.
...
scVI~\cite{lopez2018deep} and its semi-supervised extension
scANVI~\cite{xu2021scanvi} exemplify this paradigm, fitting a
...
Existing methods address these ingredients separately.
GAHIB brings graph-structured encoding, information-bottleneck
compression, and hyperbolic geometry together within one generative
framework (\cref{fig:overview}).

In this paper, we propose \textbf{GAHIB}, which integrates three
structural components into a single VAE:
...
We validate GAHIB through 11 experimental studies --- 7 comparative
benchmarks and 4 robustness-and-efficiency studies --- complemented by
a biological-interpretation analysis. The 7 \emph{comparative
benchmarks} quantify GAHIB's performance relative to prior art:
...
\end{enumerate}
A separate \emph{biological-interpretation} analysis
(\cref{sec:interpretation}) asks whether the learned latent space
captures developmental hierarchy, gene programs, cell-type homophily,
and tissue-specific Gene Ontology Biological Process enrichment
(\cref{sec:go}) on a representative subset of datasets for which
cell-type or lineage annotations are available.
...
map---the geometric foundation we adopt in GAHIB---and hyperbolic
representations have since proven effective in language settings
(Poincar\'e GloVe~\cite{tifrea2019poincareglove}) as well as in
single-cell biology (Poincar\'e maps~\cite{klimovskaia2020poincare}).
...
We construct a $k$-nearest-neighbour graph ($k{=}15$) over cells
using scanpy's neighbourhood graph on a 50-D PCA embedding with
Euclidean distance and UMAP-style connectivity weights.
A two-layer GAT encoder (hidden dimension 128, single attention head,
dropout 0.05) with BatchNorm, ReLU, and a residual link maps each
cell $\bx_i$ to variational parameters $(\bmu_i, \log\bsig_i^2)$ via
attention-weighted neighbourhood aggregation:
\begin{equation}
...
A linear encoder $W_e$ compresses $\bz \in \rea^d$ to a 2D manifold
coordinate $\bz' = W_e \bz \in \rea^2$, and a linear decoder
$W_d$ maps $\bz'$ back to $\rea^d$.
The compressed coordinate $\bz'$ is fed through the same negative-binomial
decoder used for $\bz$, yielding an auxiliary reconstruction loss
$\mathcal{L}_\text{ib} = -\log p_\text{NB}(\bx \mid \bz')$ weighted
by $\lambda_\text{ib}{=}0.5$.
Training $\bz'$ to retain reconstruction-relevant information makes
it a meaningful manifold coordinate; without this term the bottleneck
is untrained and the Lorentz distance below becomes degenerate.
...
to match the number of mini-batch updates used by MLP encoders.
Without this step the graph encoder would receive only one gradient
update per epoch versus ${\sim}16$ for the MLP, which would
systematically under-train the GAT and confound any fair comparison.
...
We query these gene lists against a curated built-in panel of
hallmark-style gene sets derived from MSigDB Biological Process
categories~\cite{ashburner2000go} via a hypergeometric test in the
spirit of gene-set enrichment
analysis~\cite{subramanian2005gsea}, applying
Benjamini--Hochberg correction at adjusted $p<0.05$.
...
Cluster labels are obtained via Leiden
clustering~\cite{traag2019leiden} (resolution\,$=$\,1.0) on
PCA$\to$UMAP neighbours computed with
Scanpy~\cite{wolf2018scanpy}, providing a consistent unsupervised
reference partition across all 53 datasets (limitations of this
proxy are discussed in \cref{sec:discussion}).
...
\paragraph{Per-comparison test.}
All method-vs-method comparisons use the two-sided
\textbf{Wilcoxon signed-rank test} across the 53 datasets.
The paired design (same dataset, different methods) removes
between-dataset variance, and the non-parametric test is used because
metric distributions across heterogeneous datasets are not guaranteed
to be normal.
Raw significance levels are reported as
\sigA~$p < 0.001$, \sigB~$p < 0.01$, \sigC~$p < 0.05$, and
\ns~not significant.

\paragraph{Multiple-comparison correction.}
Each results table constitutes a family of tests: (baseline methods
$-1$) $\times$ (metrics reported in that table), typically 30 to 100
hypotheses.
Within each table we therefore apply the
\textbf{Benjamini--Hochberg FDR procedure} at $q{=}0.05$ over the
family and flag a comparison as ``significant gain'' only when
it remains below the FDR threshold.
The significance markers reported in \cref{tab:ablation,%
tab:scdeep,tab:classical,tab:gmvae,tab:disent,tab:encoder,tab:gconv}
correspond to BH-corrected $p$-values.
Uncorrected raw $p$-values are released with the code archive for
readers who prefer a different correction.

\paragraph{Matching.}
All baselines use the same preprocessing pipeline (scanpy
filter/HVG/normalisation; identical 15-NN graph), the same
train--validation split, the same 200-epoch training budget with
patience~30, and default hyperparameters as published by each method.
Deep-learning baselines that expose a latent-dimension parameter are
set to $d{=}10$ to match GAHIB; classical DR methods use their
standard defaults.
No per-method hyperparameter search was performed.

\paragraph{Independence caveat.}
The 53 datasets come from distinct GEO / Zenodo / Figshare
accessions, different tissues, and different primary publications,
so they are plausibly independent at the dataset level.
We do not claim strict independence: three pairs of datasets share
a parent study (e.g., bccHm / sccHm), and some datasets are from
the same protocol family.
Wilcoxon results are therefore best interpreted as ``aggregate
evidence over a diverse cohort'', not as a strict i.i.d.\ hypothesis
test.
...
reduces DAV by $-$0.252: the 2D bottleneck discards technical noise and
keeps the most discriminative gene-expression patterns, which tightens
cluster separation (higher ASW, lower DAV). The IB also supplies the
structured 2D manifold coordinate~$\bz'$ required as an anchor for the
hyperbolic geometry loss in the subsequent variants.
...
\emph{Hyp without IB is ineffective for clustering}: without a
...
at $p < 0.001$.
The GAT encoder aggregates information from cell-type-homophilic
neighbours (mean attention homophily 0.812), producing pre-clustered
representations that make the IB and hyperbolic losses markedly more
effective. The GAT encoder therefore carries most of the clustering
gain, while the geometric losses refine the resulting latent geometry.
...
ARI ($p = 0.989$, ns), and scDHMap on DRE-UMAP ($p = 0.60$, ns).
...
Only the Learnable PGM variant approaches competitive clustering
(NMI\,$=$\,0.600); the remaining priors collapse to near-random
partitions (NMI\,$\leq$\,0.027). A hyperbolic prior on its own is not
enough: without neighbourhood-aware encoding and an anchored bottleneck,
the geometry has no structure to regularise (\cref{fig:gmvae}).
...
$\beta$-VAE, DIP-VAE, and TC-VAE \emph{decrease} clustering quality
below the unregularised Base VAE (TC-VAE NMI 0.404 vs.\ Base 0.647).
These regularisers impose strong constraints on the posterior,
encouraging independent or factorised latent dimensions; in scRNA-seq
data, however, the dominant sources of variation are co-regulated gene
programmes, so forcing orthogonality disrupts the representation the
encoder would otherwise learn. InfoVAE, which relaxes the
factorisation constraint, is nearly neutral (NMI 0.649 vs.\ Base
0.647), consistent with this interpretation.
GAHIB achieves NMI 0.722 ($+$0.073 over InfoVAE, $+$0.318 over TC-VAE),
indicating that a \emph{geometric} structural prior (IB+Hyp) is more
effective than posterior regularisation for single-cell clustering
(\cref{fig:disent}).
...
Stem and progenitor cells have
lower norms (closer to the origin) while differentiated cells have higher
norms (closer to boundary).

This is quantified by the stemness--norm Spearman correlation
$\rho_s$ in \cref{tab:interp}, which is positive in 45/53 datasets
(mean $\rho_s = 0.329$) and reaches significance in the majority;
the remaining 8 datasets show small-magnitude negative correlations,
consistent with non-developmental tissue composition.
...
correlated genes, and queried them against a curated hallmark-style
panel of Biological Process gene sets via a hypergeometric test.
Significance used Benjamini--Hochberg adjusted $p{<}0.05$.

Panel~(e) of \cref{fig:downstream} summarises the top-three
enriched terms per latent dimension for each tissue context.
GAHIB's latent dimensions partition biological programmes
compartmentally:
...
generic pathways.
Panel~(e) of \cref{fig:downstream} displays the top-three enriched
GO~BP terms per latent dimension for each of the four datasets,
trained as separate GAHIB models.
The enrichment is context-dependent:
...
\Cref{fig:downstream} panels~(a)--(d) summarise the gene-expression
...
trajectories along $r_i$ (panels~c,~d), indicating that GAHIB's
hyperbolic pseudotime tracks a biologically coherent gene programme
rather than a statistical artefact of the embedding.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figure14}
\caption{Downstream analysis of GAHIB.
Panels~(a)--(d) validate GAHIB's Lorentz-norm pseudotime from the
original gene expression on four datasets (Dentate gyrus,
Hematopoiesis, Setty HSPC, hESC).
...
HVG (progenitor-like; expression decreases with $r_i$), with the
Pearson~$r$ annotated.
(d)~Scatter of GAHIB pseudotime vs. the top positively-correlated
HVG (lineage-commitment; expression increases with $r_i$).
(e)~Tissue-specific GO~Biological~Process enrichment of the ten
latent dimensions for four independently trained GAHIB models;
each heatmap column is one latent dimension and each row is an
enriched term (cell colour $= -\log_{10}$ adjusted $p$-value).
Latent dimensions specialise to tissue-appropriate programmes
without cross-context leakage.}
...
top attributed genes (\cref{tab:interp}), and identifies about 22
significant GO enrichment terms per dataset on average
(34.3 among the 34 datasets with non-zero enrichment; 19 datasets
produce no enriched terms at BH-adj.\,$p<0.05$).
...
Mean GAT attention homophily is 0.812, with 46/53 datasets above
0.75 (minimum 0.458 for lung), indicating that the attention
mechanism captures biologically coherent cell--cell interactions
...
\begin{table}[!htbp]
\centering
\caption{Per-dataset interpretation summary.
Twelve representative datasets are shown; the table mean is therefore
computed over these 12 rows, while the full-cohort mean
($\rho_s{=}0.329$) reported in the text is computed over all 53 datasets.
Hom.\ = GAT attention homophily;
$\bar{r}_L$ = mean Lorentz norm;
$\rho_s$ = stemness--norm Spearman correlation (significance stars in
the column report the per-dataset Wilcoxon test: \sigA~$p<0.001$,
\sigC~$p<0.05$);
MO = marker overlap fraction; GO = significant GO enrichment terms.}
\label{tab:interp}
...
We provide four additional studies covering latent-dimension
selection, reproducibility across random seeds, computational cost,
and hyperparameter sensitivity. All studies reuse the 53 scRNA-seq
datasets and the same 20-metric evaluation protocol.

\subsection{Latent Dimension Ablation and Multi-Seed Reproducibility}
\label{sec:latdim}%
\label{sec:seeds}

\Cref{fig:latdim}(a) sweeps the latent dimension
$d\in\{3,5,10,20,50\}$ while holding the IB dimension fixed at~2 and
...
\Cref{fig:seeds}(b) retrains GAHIB with five random seeds
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.98\textwidth]{figure16}
\caption{Latent dimension ablation and multi-seed reproducibility
across 53 datasets, evaluated on eight clustering/embedding metrics
(NMI, ARI, ASW, DAV, CAL, COR, DRE (UMAP), LSE).
(a) Bars show mean per-metric score across 53 datasets as a function
of the latent dimension $d\in\{3,5,10,20,50\}$; error bars denote one
standard deviation. The default $d{=}10$ (red outline) lies near the
stable plateau before ASW degrades at $d{=}50$.
(b) Per-dataset multi-seed mean ($\pm$ std over 5 random seeds);
each dot is one dataset, coloured by its mean score, and the red
dashed line marks the overall mean.}
\label{fig:latdim}%
\label{fig:seeds}%
\end{figure*}
...
three architectures across the 53 datasets.
GAHIB requires 22.0$\pm$3.2\,s per dataset and 0.11\,GB peak memory,
a 41\% overhead over Base VAE (15.6$\pm$2.4\,s, 0.055\,GB) and 10\%
over VAE+IB+Hyp (20.0\,s, 0.059\,GB).
The additional cost comes primarily from subgraph sampling and GAT
message passing.

\Cref{fig:cost}(b) examines cell-count scaling on 10 representative
datasets, each uniformly random-subsampled to
$n\in\{500, 1000, 2000, 3000\}$ cells and retrained from scratch.
Training time is flat (22.0--22.6\,s across all four sizes) because
the per-epoch cost is dominated by a fixed number of subgraph
training steps (10 subgraphs $\times$ 512 nodes), not the dataset
size; the measurement therefore reports the steady-state cost
rather than a true asymptotic.

\Cref{fig:cost}(c) isolates the mini-batch-size effect by holding
$n$ fixed at 2\,000 cells (uniform random subsample from 5 datasets)
and sweeping the training batch size through
$\{32, 64, 128, 256, 512\}$.
Per-epoch wall-clock time is essentially constant
(0.59--0.61\,s/epoch, relative spread ${<}4\%$) across the whole
range.
This insensitivity is a direct consequence of GAHIB's subgraph-paced
schedule: each epoch processes a fixed number of 512-node subgraphs
regardless of the DataLoader batch, so the batch parameter controls
only the downstream mini-batch reduction inside each subgraph.
Practically, this means GAHIB does not require batch-size tuning and
the default of 128 can be used without loss.

\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.98\textwidth]{figure17}
\caption{Computational cost analysis.
\textbf{(a)}~Training time (bars, mean$\pm$std) and peak GPU memory
(annotations) for three architectures, aggregated across 53 datasets
at 200 epochs.
\textbf{(b)}~Cell-count scaling: GAHIB training time versus
uniform-random subsample size $n\in\{500, 1000, 2000, 3000\}$ cells,
measured on 10 representative datasets.
\textbf{(c)}~Batch-size insensitivity at fixed $n{=}2{,}000$ cells
across 5 datasets: time per epoch as a function of mini-batch size
varies by less than 4\% across $\{32,64,128,256,512\}$ because
GAHIB's epoch is paced by a fixed subgraph-sampling schedule.
Panels (a)/(b) use an NVIDIA RTX 4080 Laptop GPU;
panel (c) uses a CPU worker since batch-size comparisons are
dominated by sampling overhead rather than GPU compute.}
\label{fig:cost}
\end{figure*}
...
\subsection{A Balanced Embedding, Not a Specialist}

GAHIB's practical value is balance: it is the only method that remains
strong across clustering, geometry-preservation, and latent-structure
...
GAHIB reduces that trade-off. Compared with scVI it preserves stronger
cluster margins and latent geometry; compared with scDHMap it improves
separation while keeping the hierarchy signal; compared with classical
methods it is more reliable when tumour heterogeneity and developmental
continua coexist in the same dataset. In settings where both
neighbourhood fidelity and branching organisation matter, GAHIB is best
used as a balanced general-purpose embedding rather than as a
specialised hyperbolic model.

\subsection{The GAT Encoder Drives the Clustering Gain}
...
The origin of hyperbolic space is the unique point of minimum norm; in
tree embeddings~\cite{nickel2017poincare}---and in their single-cell
analogue Poincar\'e maps~\cite{klimovskaia2020poincare}---the root of
the hierarchy maps to the origin while leaves map to high-norm positions.
Earlier hyperbolic variational
autoencoders~\cite{mathieu2019poincarevae,nagano2019wrappednormal}
established the reparameterisation machinery on the Poincar\'e disk and
the Lorentz model, but did not couple this geometry to graph-based
encoding or to an information bottleneck.
...
The Lorentz geometry loss therefore recovers a biologically
coherent radial ordering of cell states and yields a continuous
latent coordinate that correlates with canonical stemness markers on
annotated datasets, as a by-product of the variational objective.

\subsection{Disentanglement Regularisation Hurts Single-Cell Clustering}

In the disentanglement benchmark (\cref{sec:benchmark_disent}), every
regulariser tested (\(\beta\)-VAE, DIP-VAE, TC-VAE, InfoVAE)
\emph{degrades} clustering quality relative to the unregularised
Base VAE, with TC-VAE producing the most severe collapse
(NMI 0.404, $-$0.243 vs.\ Base VAE).
...
InfoVAE~\cite{zhao2019infovae}, which relaxes the factorisation
constraint, is almost neutral (NMI 0.649 vs.\ Base 0.647), consistent
with this interpretation: minimal factorisation pressure preserves
correlation structure.
GAHIB's IB layer, by contrast, does not impose independence. It imposes
a \emph{geometric} constraint---a compact 2D projection that still
reconstructs the input---which selects for a low-dimensional manifold
without disrupting the correlation structure of the full 10D latent
code. Geometric compression rather than statistical independence is
what lets GAHIB sustain both strong clustering (NMI 0.722) and
high-fidelity dimensionality reduction (DRE 0.700).
...
The Lorentz norm's correlation with stemness ($\rho_s = 0.329$ mean;
...
A latent dimension from the brain-trained model is never enriched for
blood biology, and vice versa---%
...
Multi-seed experiments (\cref{sec:seeds}) show that GAHIB's clustering
performance varies by less than 3\% across 5 random seeds, so the
reported gains are not a consequence of favourable seed selection.
...
mid-range RTX 4080 Laptop GPU with 0.11\,GB peak memory---a 41\%
overhead over Base VAE for a substantial performance gain.
Wall-clock time is flat with cell count (the training loop samples a
fixed number of subgraphs per epoch) and per-epoch time is also flat
across mini-batch sizes from 32 to 512, so GAHIB does not require
batch-size tuning and has headroom to scale to atlas-size datasets
without retraining the graph.
...
graph-sampling strategies such as
GraphSAINT~\cite{zeng2020graphsaint} or
Cluster-GCN~\cite{chiang2019clustergcn} combined with
...
We presented GAHIB, a graph-attention VAE with a hyperbolic
information bottleneck for single-cell latent representation learning.
The model combines GAT encoding, information bottleneck compression,
and Lorentz hyperbolic geometry in a single framework, with the
bottleneck providing the target used by the hyperbolic term.

Through 11 experimental studies on 53 scRNA-seq datasets, we
demonstrated that:
\begin{enumerate}[leftmargin=*,itemsep=2pt]
...
\item GAHIB matches or exceeds 7 deep-learning, 5 classical DR, 5
  geometric-VAE, and 4 disentanglement baselines on clustering and
  dimensionality-reduction fidelity, with BH-corrected improvements in
  the majority of pairwise comparisons.
\item GAHIB's latent space captures developmental hierarchy on
  annotated datasets: hyperbolic radii correlate with stemness
  ($\rho_s{=}0.329$ mean across 53 datasets, up to 0.673 on
  developmental systems), decoder Jacobians recover 31.5\% of marker
  genes, and GAT attention shows 81.2\% mean cell-type homophily
  (46/53 datasets above 0.75).
\item GAHIB remains stable across repeated runs and tuning settings:
  performance varies by less than 3\% across 5 random seeds, remains
  stable across hyperparameter sweeps (NMI spread $\leq 0.044$), and
  trains in 22\,s/dataset with flat scaling across cell counts.
\end{enumerate}

Taken together, these results indicate that coupling graph-structured
encoding with hyperbolic geometry through an information bottleneck is
a useful design for single-cell latent representation learning. More
broadly, the study suggests that neighbourhood structure and
hierarchical geometry can be treated as complementary, rather than
competing, inductive biases in future single-cell generative models.
...
\paragraph{Code.}
The core GAHIB library (model, metrics, interpretation utilities),
the 11 experiment runners, and the test suite are publicly available
under an open-source licence at
\url{https://github.com/PeterPonyu/GAHIB}. The visualisation pipeline
and the manuscript sources are maintained locally and can be obtained
...
Zeyu Fu, Jiawei Fu and Xiaoxia Wang contributed equally to this work.
...
\textbf{Jiawei Fu:} methodology, investigation, data curation, formal
analysis, writing---review and editing.
\textbf{Xiaoxia Wang:} investigation, data curation, validation,
writing---review and editing.
\textbf{Yiyao Liu:} supervision, project administration, resources,
funding acquisition, writing---review and editing.
\textbf{Tianfei Ran:} conceptualization, supervision, project
...
This work was supported by the Special Project for Enhancing the
Scientific and Technological Innovation Capacity of the Army Medical
University (No.~2023XJS34).
...
The authors declare no competing financial or personal interests that
...
The authors used AI-based coding-assistance tools (large language
models) during \emph{code development} only---specifically for
boilerplate scaffolding, routine debugging, and visualisation helper
generation in the GAHIB implementation and experiment pipelines.
AI tools were \textbf{not} used for writing, drafting, editing, or
refining any portion of this manuscript.
All scientific contributions, analyses, experimental designs,
interpretations, manuscript text, and conclusions are the original
work of the authors, who take full responsibility for all content.
...
G.~X.~Y. Zheng, J.~M. Terry, P.~Belgrader, et~al., Massively parallel digital
  transcriptional profiling of single cells, Nature Communications 8 (2017)
  14049.
\newblock \href {https://doi.org/10.1038/ncomms14049}
  {\path{doi:10.1038/ncomms14049}}.

\bibitem{wolf2018scanpy}
F.~A. Wolf, P.~Angerer, F.~J. Theis, Scanpy: large-scale single-cell gene
  expression data analysis, Genome Biology 19~(1) (2018) 15.
\newblock \href {https://doi.org/10.1186/s13059-017-1382-0}
  {\path{doi:10.1186/s13059-017-1382-0}}.

\bibitem{traag2019leiden}
V.~A. Traag, L.~Waltman, N.~J. Van~Eck, From louvain to leiden: guaranteeing
  well-connected communities, Scientific Reports 9~(1) (2019) 5233.
\newblock \href {https://doi.org/10.1038/s41598-019-41695-z}
  {\path{doi:10.1038/s41598-019-41695-z}}.

\bibitem{lopez2018deep}
R.~Lopez, J.~Regier, M.~B. Cole, M.~I. Jordan, N.~Yosef, Deep generative
  modeling for single-cell transcriptomics, Nature Methods 15~(12) (2018)
  1053--1058.
\newblock \href {https://doi.org/10.1038/s41592-018-0229-2}
  {\path{doi:10.1038/s41592-018-0229-2}}.

\bibitem{gronbech2020scvae}
C.~H. Gr{\o}nbech, M.~F. Vording, P.~N. Timshel, C.~K. S{\o}nderby, T.~H. Pers,
  O.~Winther, scvae: variational auto-encoders for single-cell gene expression
  data, Bioinformatics 36~(16) (2020) 4415--4422.
\newblock \href {https://doi.org/10.1093/bioinformatics/btaa293}
  {\path{doi:10.1093/bioinformatics/btaa293}}.

\bibitem{nickel2017poincare}
M.~Nickel, D.~Kiela,
  \href{https://proceedings.neurips.cc/paper_files/paper/2017/hash/59dfa2df42d9e3d41f5b02bfc32229dd-Abstract.html}{Poincar{\'e}
  embeddings for learning hierarchical representations}, in: Advances in Neural
  Information Processing Systems 30, Vol.~30, 2017, pp. 6338--6347.
\newline\urlprefix\url{https://proceedings.neurips.cc/paper_files/paper/2017/hash/59dfa2df42d9e3d41f5b02bfc32229dd-Abstract.html}

\bibitem{klimovskaia2020poincare}
A.~Klimovskaia, D.~Lopez-Paz, L.~Bottou, M.~Nickel,
  Poincar{\'e} maps for analyzing complex hierarchies in single-cell data,
  Nature Communications 11~(1) (2020) 2966.
\newblock \href {https://doi.org/10.1038/s41467-020-16822-4}
  {\path{doi:10.1038/s41467-020-16822-4}}.

\bibitem{tifrea2019poincareglove}
A.~Tifrea, G.~B{\'e}cigneul, O.-E. Ganea,
  Poincar{\'e} {GloVe}: Hyperbolic word embeddings, in: International
  Conference on Learning Representations, 2019.

\bibitem{xu2021scanvi}
C.~Xu, R.~Lopez, E.~Mehlman, J.~Regier, M.~I. Jordan, N.~Yosef,
  Probabilistic harmonization and annotation of single-cell transcriptomics
  data with deep generative models, Molecular Systems Biology 17~(1) (2021)
  e9620.
\newblock \href {https://doi.org/10.15252/msb.20209620}
  {\path{doi:10.15252/msb.20209620}}.

\bibitem{ganea2018hyperbolic}
O.~Ganea, G.~B{\'e}cigneul, T.~Hofmann,
  \href{https://proceedings.neurips.cc/paper/2018/hash/dbab2adc8f9d078009ee3fa810bea142-Abstract.html}{Hyperbolic
  neural networks}, in: Advances in Neural Information Processing Systems 31,
  Vol.~31, 2018, pp. 5350--5360.
\newline\urlprefix\url{https://proceedings.neurips.cc/paper/2018/hash/dbab2adc8f9d078009ee3fa810bea142-Abstract.html}

\bibitem{bayer2023gmvae}
F.~Bayer, T.~Silander, {GM-VAE}: Representation learning with variational
  autoencoders for genomic data using geometric priors, in: NeurIPS 2023
  Workshop on Geometrical and Topological Representation Learning, 2023.

\bibitem{wang2021scgnn}
J.~Wang, A.~Ma, Y.~Chang, J.~Gong, Y.~Jiang, R.~Qi, C.~Wang, H.~Fu, Q.~Ma,
  D.~Xu, scgnn is a novel graph neural network framework for single-cell
  rna-seq analyses, Nature Communications 12 (2021) 1882.
\newblock \href {https://doi.org/10.1038/s41467-021-22197-x}
  {\path{doi:10.1038/s41467-021-22197-x}}.

\bibitem{tian2023scgcc}
S.-W. Tian, J.-C. Ni, Y.-T. Wang, C.-H. Zheng, C.-M. Ji, sc{GCC}: Graph
  contrastive clustering with neighborhood augmentations for {scRNA-Seq} data
  analysis, IEEE Journal of Biomedical and Health Informatics 27~(12) (2023)
  6133--6143.
\newblock \href {https://doi.org/10.1109/JBHI.2023.3319551}
  {\path{doi:10.1109/JBHI.2023.3319551}}.

\bibitem{tian2023scdhmap}
T.~Tian, C.~Zhong, X.~Lin, Z.~Wei, H.~Hakonarson, Complex hierarchical
  structures in single-cell genomics data unveiled by deep hyperbolic manifold
  learning, Genome Research 33~(2) (2023) 232--246.
\newblock \href {https://doi.org/10.1101/gr.277068.122}
  {\path{doi:10.1101/gr.277068.122}}.

\bibitem{choi2023sivae}
Y.~Choi, R.~Li, G.~Quon, si{VAE}: interpretable deep generative models for
  single-cell transcriptomes, Genome Biology 24~(1) (2023) 29.
\newblock \href {https://doi.org/10.1186/s13059-023-02850-y}
  {\path{doi:10.1186/s13059-023-02850-y}}.

\bibitem{alemi2017deep}
A.~A. Alemi, I.~Fischer, J.~V. Dillon, K.~Murphy,
  \href{https://openreview.net/forum?id=HyxQzBceg}{Deep variational information
  bottleneck}, in: International Conference on Learning Representations, 2017.
\newline\urlprefix\url{https://openreview.net/forum?id=HyxQzBceg}

\bibitem{han2022clear}
W.~Han, Y.~Cheng, J.~Chen, H.~Zhong, Z.~Hu, S.~Chen, L.~Zong, L.~Hong, T.-F.
  Chan, I.~King, et~al., Self-supervised contrastive learning for integrative
  single cell {RNA-seq} data analysis, Briefings in Bioinformatics 23~(5)
  (2022) bbac377.
\newblock \href {https://doi.org/10.1093/bib/bbac377}
  {\path{doi:10.1093/bib/bbac377}}.

\bibitem{an2024scdac}
S.~An, J.~Shi, R.~Liu, Y.~Chen, J.~Wang, S.~Hu, X.~Xia, G.~Dong, sc{DAC}: deep
  adaptive clustering of single-cell transcriptomic data with coupled
  autoencoder and {Dirichlet} process mixture model, Bioinformatics 40~(4)
  (2024) btae198.
\newblock \href {https://doi.org/10.1093/bioinformatics/btae198}
  {\path{doi:10.1093/bioinformatics/btae198}}.

\bibitem{xiong2022scalex}
L.~Xiong, K.~Tian, Y.~Li, W.~Ning, X.~Gao, Q.~C. Zhang, Online single-cell data
  integration through projecting heterogeneous datasets into a common
  cell-embedding space, Nature Communications 13 (2022) 6118.
\newblock \href {https://doi.org/10.1038/s41467-022-33758-z}
  {\path{doi:10.1038/s41467-022-33758-z}}.

\bibitem{velickovic2018graph}
P.~Veli{\v{c}}kovi{\'c}, G.~Cucurull, A.~Casanova, A.~Romero, P.~Li{\`o},
  Y.~Bengio, \href{https://openreview.net/forum?id=rJXMpikCZ}{Graph attention
  networks}, in: International Conference on Learning Representations, 2018.
\newline\urlprefix\url{https://openreview.net/forum?id=rJXMpikCZ}

\bibitem{buterez2022cellvgae}
D.~Buterez, I.~Bica, I.~Tariq, H.~Andr{\'e}s-Terr{\'e}, P.~Li{\`o}, {CellVGAE}:
  an unsupervised {scRNA-seq} analysis workflow with graph attention networks,
  Bioinformatics 38~(5) (2022) 1277--1286.
\newblock \href {https://doi.org/10.1093/bioinformatics/btab804}
  {\path{doi:10.1093/bioinformatics/btab804}}.

\bibitem{tishby2000information}
N.~Tishby, F.~C. Pereira, W.~Bialek,
  \href{https://arxiv.org/abs/physics/0004057}{The information bottleneck
  method}, arXiv preprint physics/0004057 (2000).
\newline\urlprefix\url{https://arxiv.org/abs/physics/0004057}

\bibitem{higgins2017betavae}
I.~Higgins, L.~Matthey, A.~Pal, C.~Burgess, X.~Glorot, M.~Botvinick,
  S.~Mohamed, A.~Lerchner,
  \href{https://openreview.net/forum?id=Sy2fzU9gl}{$\beta$-{VAE}: Learning
  basic visual concepts with a constrained variational framework}, in:
  International Conference on Learning Representations, 2017.
\newline\urlprefix\url{https://openreview.net/forum?id=Sy2fzU9gl}

\bibitem{kumar2018dipvae}
A.~Kumar, P.~Sattigeri, A.~Balakrishnan,
  \href{https://openreview.net/forum?id=H1kG7GZAW}{Variational inference of
  disentangled latent concepts from unlabeled observations}, in: International
  Conference on Learning Representations, 2018.
\newline\urlprefix\url{https://openreview.net/forum?id=H1kG7GZAW}

\bibitem{chen2018isolating}
R.~T.~Q. Chen, X.~Li, R.~B. Grosse, D.~K. Duvenaud, Isolating sources of
  disentanglement in variational autoencoders, in: Advances in Neural
  Information Processing Systems 31, Vol.~31, 2018, pp. 2615--2625.

\bibitem{kim2018factor}
H.~Kim, A.~Mnih,
  \href{https://proceedings.mlr.press/v80/kim18b.html}{Disentangling by
  factorising}, in: Proceedings of the 35th International Conference on Machine
  Learning, Vol.~80, 2018, pp. 2649--2658.
\newline\urlprefix\url{https://proceedings.mlr.press/v80/kim18b.html}

\bibitem{zhao2019infovae}
S.~Zhao, J.~Song, S.~Ermon, Infovae: Balancing learning and inference in
  variational autoencoders, in: Proceedings of the AAAI Conference on
  Artificial Intelligence, Vol.~33, 2019, pp. 5885--5892.
\newblock \href {https://doi.org/10.1609/aaai.v33i01.33015885}
  {\path{doi:10.1609/aaai.v33i01.33015885}}.

\bibitem{cao2020cellblast}
Z.-J. Cao, L.~Wei, S.~Lu, D.-C. Yang, G.~Gao, Searching large-scale {scRNA-seq}
  databases via unbiased cell embedding with {Cell BLAST}, Nature
  Communications 11 (2020) 3458.
\newblock \href {https://doi.org/10.1038/s41467-020-17281-7}
  {\path{doi:10.1038/s41467-020-17281-7}}.

\bibitem{tian2019scdeepcluster}
T.~Tian, J.~Wan, Q.~Song, Z.~Wei, Clustering single-cell {RNA-seq} data with a
  model-based deep learning approach, Nature Machine Intelligence 1~(4) (2019)
  191--198.
\newblock \href {https://doi.org/10.1038/s42256-019-0037-0}
  {\path{doi:10.1038/s42256-019-0037-0}}.

\bibitem{luecken2019bestpractices}
M.~D. Luecken, F.~J. Theis, Current best practices in single-cell
  {RNA-seq} analysis: a tutorial, Molecular Systems Biology 15~(6) (2019)
  e8746.
\newblock \href {https://doi.org/10.15252/msb.20188746}
  {\path{doi:10.15252/msb.20188746}}.

\bibitem{lahnemann2020grandchallenges}
D.~L{\"a}hnemann, J.~K{\"o}ster, E.~Szczurek, et~al.,
  Eleven grand challenges in single-cell data science,
  Genome Biology 21 (2020) 31.
\newblock \href {https://doi.org/10.1186/s13059-020-1926-6}
  {\path{doi:10.1186/s13059-020-1926-6}}.

\bibitem{hafemeister2019sctransform}
C.~Hafemeister, R.~Satija, Normalization and variance stabilization of
  single-cell {RNA-seq} data using regularized negative binomial regression,
  Genome Biology 20 (2019) 296.
\newblock \href {https://doi.org/10.1186/s13059-019-1874-1}
  {\path{doi:10.1186/s13059-019-1874-1}}.

\bibitem{svensson2020droplet}
V.~Svensson, Droplet scRNA-seq is not zero-inflated,
  Nature Biotechnology 38~(2) (2020) 147--150.
\newblock \href {https://doi.org/10.1038/s41587-019-0379-5}
  {\path{doi:10.1038/s41587-019-0379-5}}.

\bibitem{trapnell2014monocle}
C.~Trapnell, D.~Cacchiarelli, J.~Grimsby, P.~Pokharel, S.~Li, M.~Morse,
  N.~J. Lennon, K.~J. Livak, T.~S. Mikkelsen, J.~L. Rinn,
  The dynamics and regulators of cell fate decisions are revealed by
  pseudotemporal ordering of single cells,
  Nature Biotechnology 32~(4) (2014) 381--386.
\newblock \href {https://doi.org/10.1038/nbt.2859}
  {\path{doi:10.1038/nbt.2859}}.

\bibitem{haghverdi2016dpt}
L.~Haghverdi, M.~B{\"u}ttner, F.~A. Wolf, F.~Buettner, F.~J. Theis,
  Diffusion pseudotime robustly reconstructs lineage branching,
  Nature Methods 13~(10) (2016) 845--848.
\newblock \href {https://doi.org/10.1038/nmeth.3971}
  {\path{doi:10.1038/nmeth.3971}}.

\bibitem{saelens2019trajectory}
W.~Saelens, R.~Cannoodt, H.~Todorov, Y.~Saeys,
  A comparison of single-cell trajectory inference methods,
  Nature Biotechnology 37~(5) (2019) 547--554.
\newblock \href {https://doi.org/10.1038/s41587-019-0071-9}
  {\path{doi:10.1038/s41587-019-0071-9}}.

\bibitem{ashburner2000go}
M.~Ashburner, C.~A. Ball, J.~A. Blake, D.~Botstein, H.~Butler, et~al.,
  Gene ontology: tool for the unification of biology,
  Nature Genetics 25~(1) (2000) 25--29.
\newblock \href {https://doi.org/10.1038/75556}
  {\path{doi:10.1038/75556}}.
...
A.~Subramanian, P.~Tamayo, V.~K. Mootha, S.~Mukherjee, B.~L. Ebert,
  ...
  Proceedings of the National Academy of Sciences 102~(43) (2005)
  15545--15550.
\newblock \href {https://doi.org/10.1073/pnas.0506580102}
  {\path{doi:10.1073/pnas.0506580102}}.

\bibitem{mathieu2019poincarevae}
E.~Mathieu, C.~Le~Lan, C.~J. Maddison, R.~Tomioka, Y.~W. Teh,
  Continuous hierarchical representations with {Poincar{\'e}}
  variational auto-encoders, in: Advances in Neural Information
  Processing Systems 32, 2019, pp. 12565--12576.

\bibitem{nagano2019wrappednormal}
Y.~Nagano, S.~Yamaguchi, Y.~Fujita, M.~Koyama,
  A wrapped normal distribution on hyperbolic space for gradient-based
  learning, in: Proceedings of the 36th International Conference on
  Machine Learning, Vol.~97, 2019, pp. 4693--4702.

\bibitem{zeng2020graphsaint}
H.~Zeng, H.~Zhou, A.~Srivastava, R.~Kannan, V.~Prasanna,
  {GraphSAINT}: Graph sampling based inductive learning method, in:
  International Conference on Learning Representations, 2020.

\bibitem{chiang2019clustergcn}
W.-L. Chiang, X.~Liu, S.~Si, Y.~Li, S.~Bengio, C.-J. Hsieh,
  {Cluster-GCN}: An efficient algorithm for training deep and large
  graph convolutional networks, in: Proceedings of the 25th ACM SIGKDD
  International Conference on Knowledge Discovery \& Data Mining, 2019,
  pp. 257--266.
\newblock \href {https://doi.org/10.1145/3292500.3330925}
  {\path{doi:10.1145/3292500.3330925}}.
