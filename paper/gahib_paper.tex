...
disentanglement baselines on clustering, embedding-quality, and
latent-space metrics, with statistically significant gains over
the majority of pairwise comparisons (paired Wilcoxon signed-rank,
Benjamini--Hochberg corrected). A component ablation confirms
that each architectural term contributes complementary signal,
and the learned latent space recovers a biologically coherent
radial ordering of cell states on datasets with lineage annotation.
The method is reproducible across random seeds, robust across its
four key hyperparameters, and trains in tens of seconds per dataset
on a single mid-range GPU.
\end{abstract}
