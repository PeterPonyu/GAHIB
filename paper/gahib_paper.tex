...
% ── Abstract ──────────────────────────────────────────────────────────────
\begin{abstract}
No existing single-cell RNA-sequencing (scRNA-seq) variational
autoencoder jointly models the local cell-graph neighbourhood
structure and the hierarchical, tree-like organisation of cell
states inside a single variational objective: prior methods either
use Euclidean MLP encoders that ignore the cell graph, or place a
hyperbolic prior on the latent code without neighbourhood-aware
encoding, or compress the bottleneck without a geometric anchor.
We introduce \textbf{GAHIB} (\emph{Graph Attention VAE with a
Hyperbolic Information Bottleneck}), which couples a graph-attention
encoder, a 2D information bottleneck, and a Lorentz-hyperbolic
geometry loss so that each component enables the others: the
bottleneck supplies a stable target for the hyperbolic distance,
and the graph encoder sharpens both geometric terms through
neighbourhood-aware representations.
Across 53 scRNA-seq datasets, 11 studies, and 20 metrics, GAHIB
matches or exceeds deep-learning, classical, geometric-VAE, and
disentanglement baselines on clustering (NMI, ARI, ASW), embedding
quality (DRE-UMAP, DRE-tSNE), and latent-space evaluation (LSE)
metrics, with statistically significant gains over the majority of
pairwise comparisons under paired Wilcoxon signed-rank tests
(Benjamini--Hochberg corrected).
A component ablation confirms that each architectural term
contributes complementary signal.
Multi-seed runs show ${<}$3\% standard deviation across metrics, and
hyperparameter sensitivity is bounded within $\pm$5\% across four
coefficients.
At fixed 200 epochs, GAHIB trains in 22.0$\pm$3.2\,s per dataset
(1.4$\times$ the cost of a plain VAE at 15.6\,s); because the
training loop is paced by a fixed subgraph schedule, per-epoch cost
varies by less than 4\% across mini-batch sizes from 32 to 512.
\end{abstract}
...
To our knowledge, \emph{no existing method jointly combines}
graph-structured encoding, information-bottleneck compression, and
hyperbolic geometry within a unified generative framework; this is the
gap that GAHIB addresses (\cref{fig:overview}).
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.98\textwidth]{latent_dim_and_seed}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.98\textwidth]{computational_cost}
...
\item GAHIB matches or exceeds 7 deep-learning, 5 classical DR, 5
  ...
  dimensionality-reduction fidelity, with BH-corrected significant
  gains in the majority of pairwise comparisons.
