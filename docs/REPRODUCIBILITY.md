...
### Deep-learning benchmark config audit

`experiments/run_sc_deeplearning_benchmark.py` uses `EPOCHS=200` and
`BATCH_SIZE=128` for the selected benchmark datasets discovered by
`experiments/exp_utils.py`.

- scVI: `max_epochs=200`, early stopping enabled, patience `25`,
  `lr=1e-3`, `n_latent=10`, `n_hidden=128`, `n_layers=2`.
- Unified external-benchmarker models: dense normalized HVG input, 85/15
  train/validation split with seed `42`, dataloaders at batch size `128`,
  `latent_dim=10`, `epochs=200`, patience `25`, and per-model constructor
  dimensions from `MODEL_KWARGS`.
- Graph-attention style baselines: `scGAC-style`, `SCEA-style`, and
  `scVAG-style` run transparent in-tree PyTorch routes, not exact upstream
  executions. Their explicit comparable configs are recorded in
  `*_external_status.csv` under `training_config`; see
  `experiments/external_baselines/README.md`.
- GAHIB: `hidden_dim=128`, `latent_dim=10`, `lr=1e-4`, `epochs=200`,
  patience `30`, early stopping enabled, `compute_metrics=False`, and
  `GAHIB_CONFIG` (`recon=1.0`, `irecon=0.5`, `lorentz=5.0`, `beta=0.1`,
  graph/GAT encoder).

Labels are generated by Leiden clustering on preprocessed data and are used for
evaluation. For the graph-attention style routes, labels may only determine the
cluster count needed by unsupervised losses; tests assert they are not used as
supervised targets.
